...
setpoint = 99.860   # La posición deseada en el eje z (puedes ajustar esto según tu configuración)

# Inicialización de driver servos.
sda = Pin(2)
scl = Pin(3)
id = 1
...
# Posición firme del robot
servo.position(index=0, degrees=85) 
servo.position(index=1, degrees=102) 
servo.position(index=2, degrees=70) 
servo.position(index=3, degrees=82) 
servo.position(index=4, degrees=73)
servo.position(index=5, degrees=107)
...
try:
    ...
    file.write("Tiempo (s),Setpoint,Salida PID\n")
    ...
    while True:
        ...
        if uart.any():
            ...
            if command == b'1':
                servo.position(index=0, degrees=105)  # Paso derecha
                servo.position(index=4, degrees=80)
                servo.position(index=5, degrees=100)
                utime.sleep_ms(300)
                servo.position(index=1, degrees=82)
                utime.sleep_ms(300)
                servo.position(index=0, degrees=85)
                servo.position(index=1, degrees=102)
                servo.position(index=4, degrees=73)
        ...
        # Registrar tiempo, setpoint y salida del PID en el archivo CSV
        current_time = utime.time() - start_time
        try:
            file.write("{},{:.3f},{:.3f}\n".format(current_time, setpoint, measured_value))
    ...
    print("Error al abrir o escribir en el archivo CSV:", e)
finally:
    file.close()
