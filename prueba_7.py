...
# Variables del PID
Kp = 0.9  # Ganancia proporcional
Ki = 0.01  # Ganancia integral
Kd = 0.9  # Ganancia derivativa

previous_error = 0.0
integral = 0.0

PID_MIN = -0.30  # Límite mínimo de salida del PID
PID_MAX = 0.30   # Límite máximo de salida del PID
...
setpoint = 99.84  # La posición deseada en el eje z (puedes ajustar esto según tu configuración)
...
while True:
    ...
    data += 100
    measured_value = data
    print("Measured_value: {:.2f} g".format(measured_value))
    ...
    angle_servo_4 = map_value(pid_output, PID_MIN, PID_MAX, 72, 68)
    angle_servo_5 = map_value(pid_output, PID_MIN, PID_MAX, 108, 112)  
    ...
    if uart.any():
        ...
        if command == b'1':
            ...
            servo.position(index=1, degrees=90) 
            servo.position(index=2, degrees=70) 
            servo.position(index=3, degrees=78) 
